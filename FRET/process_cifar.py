...
class MultiCorruptionDataset:
    def __init__(self, data_dir, corruption, transform=None):
        ...
        self.labels.append(np.load(label_path))

    def __len__(self):
        return len(self.data[0])

    def __getitem__(self, idx):
        data_item = [Image.fromarray(data[idx]) for data in self.data]
        label_item = self.labels[0][idx] 
        if self.transform:
            data_item = [self.transform(data) for data in data_item]
        return data_item, label_item


def process_npy2Img(data_dir, corruptions, transform=None):
    '''
    save the results into the Image.
    for cifar-10-c or cifar-100-c, we parse data according to the severity label and save each in the corresponding directory.
    '''
    for corruption in corruptions:
        dataset = MultiCorruptionDataset(data_dir, corruption, transform=transform)

        for idx, (data_items, label) in enumerate(dataset):
            corruption_dir = os.path.join(data_dir, corruption)
            if not os.path.exists(corruption_dir):
                os.makedirs(corruption_dir)

            label_dir = os.path.join(corruption_dir, str(label))
        ...
        print(f'npy2Img:corruption {corruption} success!')
...
if __name__=="__main__":
    ...
    print('process_corrupted_data end')
    gc.collect()
    npy_dir = os.path.join(cor_dir, 'corrupted','severity-5') # Select the severity level for format conversion. 
    process_npy2Img(npy_dir, corruptions, transform=transforms.ToTensor()) #  Convert npy data to image data
